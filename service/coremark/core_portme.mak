...
LOAD = adb -s $(ANDROID_SERIAL) push $(OUTFILE) /data/local/CoreMark/coremark.exe 1>/dev/null 2>/dev/null
RUN =  adb -s $(ANDROID_SERIAL) shell /data/local/CoreMark/coremark.exe
#RUN = 
#adb -s $ANDROID_SERIAL push coremark.exe

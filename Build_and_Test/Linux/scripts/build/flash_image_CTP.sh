...
export ANDROID_SERIAL=RHBEC245500390
...
baseDir=/gnu_cw_hosts/msticlxl101_users/${USER}/AndroidTesting

if [ ! -f ${baseDir}/builds/images/${image}/redhookbay_$bit/redhookbay-ota-eng.${name}.zip ]; then
 echo "Image ${version}_${bit} ${special} of ${cur_date} not found!"
...
flash_on_phone () {
    ...
    sleep 3
    fastboot_ctp flash update  ${baseDir}/builds/images/$1/redhookbay_$bit/redhookbay-ota-eng.$name.zip
#    fastboot_ctp flash system  ${baseDir}/builds/images/$1/redhookbay_32/system.img
    sleep 3
...
prepare_phone () {
    ...
    adb remount
    sleep 1
    adb shell 'sed -e "s/redhookbay/device2D7DC688/g" -i /system/build.prop'
    sleep 1
    adb shell 'sed -e "s/clovertrail/device2D7DC688/g" -i /system/build.prop'
    sleep 1
